...
public:
    ...
    void Run() override
    {
        ...
        {
            if (Interpolate(percent_getup, rl.now_state.motor_state.q, rl.params.Get<std::vector<float>>("default_dof_pos"), 0.0f, "Getting up", true)) return;
        }
    }
...
class FSM0315Factory : public FSMFactory
{
public:
    FSM0315Factory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<fsm_0315_namespace::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<fsm_0315_namespace::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<fsm_0315_namespace::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<fsm_0315_namespace::RLFSMStateRLLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "0315"; }
...
REGISTER_FSM_FACTORY(FSM0315Factory, "RLFSMStatePassive")
